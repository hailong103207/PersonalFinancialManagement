 ...
 */
public class tabBaoCaoThuNhap extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v = inflater.inflate(R.layout.fragment_tab_baocaothuthap, container, false);
        AnhXa(v);
    ...
    public void LayDuongDanFilePhanLoai(String ngaymins, String ngaymaxs) {
        ...
        //cho phần apdapter
        File[] DanhSachPhanLoaiChi = modun.ThuNhap.listFiles();//Lấy danh sách folder phân loại\
        for (File filePhanLoai : DanhSachPhanLoaiChi)
        {
            //filePhanLoai là folder Phân loại
            List<clsMucTieu> clsMucTieuList1 = null;
            try {
                clsMucTieuList1 = LayDuLieu_MucTieu_Chi(filePhanLoai, ngaymins, ngaymaxs);
            } catch (ParseException e) {
                e.printStackTrace();
            }
            TinhTongChi += tongsotien_chi;//tính tổng dần của toàn bộ
            bien_tongtienchi = tongsotien_chi;
            File DataIcon = new File(modun.ThuThap_Icon, filePhanLoai.getName().trim() + ".txt");
        ...
        baoCaoTaiChinhActivity.getTvThu().setText((String.format("%,d", Long.parseLong(String.valueOf(Math.round(TinhTongChi))))).replace(".", ",").replace(",", ","));
        //TInh phan tram
